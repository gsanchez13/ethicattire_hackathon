...
class Item extends React.Component {
  ...
  async componentDidMount() {
    let fabResponse = await axios.get('/fabrics');
    let tyResponse = await axios.get('/items/types');
  ...
  handleSubmit = async (event) => {
    ...
    try {
      ...
      const post = axios.post(`/items`, { item_img: res.data.imageUrl, fabric_id: fabChoice, clothes_id: typeChoice, user_id: 1, color: colorChoice })
    } catch (err) {
      console.log(err)
    }
